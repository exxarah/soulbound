...
namespace Core.Unity.Utils
{
                                     ...
                                     ICanvasRaycastFilter
    {
        ...
        bool ICanvasRaycastFilter.IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera)
        {
            ...
            {
                Log.Error(LogCategory.UI, "Using alphaHitTestMinimumThreshold greater than 0 on Image whose sprite texture cannot be read. " +
        ...
        private void GenerateSlicedFilledSprite(VertexHelper vh)
        {
            ...
            for (int x = 0; x < 3; x++)
            {
                ...
                for (int y = 0; y < 3; y++)
                {
                    ...
                    Vector4 vertices = new(s_SlicedVertices[x].x, s_SlicedVertices[y].y, s_SlicedVertices[x2].x,
